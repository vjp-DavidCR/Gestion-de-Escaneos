...
if [ $Cancelado -eq 0 ]
  then
    ...
    dialog --backtitle "Proyecto Shell Script - David Corchado, Mario Carreras, Ismael Lopez y Jaime Gómez" --title "Seleccionar Informe de Escaneo de VULNERABILIDADES" --yesno "¿Desea seleccionar otro INFORME?" 0 0 2>temp
    Cancelado=$?
    Eleccion=`cat temp` ; rm temp
    if [ $Cancelado -ne 0 ]
      then break=1
